...
class SimCLR(nn.Module):
    ...
    def dynamic_contrastive_loss_precalc(self, hidden1, hidden2):
        # Get (normalized) hidden1 and hidden2.
        hidden1, hidden2 = F.normalize(hidden1, p=2, dim=1), F.normalize(hidden2, p=2, dim=1)
        return hidden1, hidden2

    def dynamic_contrastive_loss_2(self, hidden1, hidden2, hidden1_large, hidden2_large, index=None, gamma=0.9):
        # Get (normalized) hidden1 and hidden2.
        batch_size = hidden1.shape[0]
        enlarged_batch_size = hidden1_large.shape[0]
        ...
        u2 = (1 - gamma) * self.u[index.cpu()].cuda() + gamma * torch.sum(neg_logits2, dim=1, keepdim=True) / (
                    2 * (batch_size - 1))
        return u1, u2, index, neg_logits1, neg_logits2, batch_size, labels, logits_ab_aa, logits_ba_bb

    def dynamic_contrastive_loss_3(self, u1, u2, u1_large, u2_large, index_large, neg_logits1, neg_logits2, batch_size, labels, logits_ab_aa, logits_ba_bb):
        self.u[index_large.cpu()] = (u1_large.detach().cpu() + u2_large.detach().cpu()) / 2
    ...
    def forward(self, x1, x2, index, gamma):
        ...
        hidden1, hidden2 = self.dynamic_contrastive_loss_precalc(h1, h2, index, gamma)
        return hidden1, hidden2
