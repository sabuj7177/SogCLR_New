...
from sogclr.MDL import MultiEpochsDataLoader
...
parser.add_argument('--start-weight-save', action='store_true')
...
def main():
    ...
    print("Total available gpus " + str(ngpus_per_node))
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    logdir = '%s_%s_bs_%s_E_%s_%s'%(args.data_name, args.arch, global_batch_size, args.epochs, args.optimizer)
    ...
    if args.start_weight_save:
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank == 0):  # only the first GPU saves checkpoint
            save_checkpoint({
                ...
                'u': model.module.u,
            }, is_best=False, filename='checkpoint_0.pth.tar')
        return
    else:
        if os.path.isfile('checkpoint_0.pth.tar'):
            print("=> loading initial checkpoint '{}'".format('checkpoint_0.pth.tar'))
            if args.gpu is None:
                checkpoint = torch.load(args.resume)
            else:
                # Map model to be loaded to specified single gpu.
    ...
    image_size = {'imagenet100':224, 'imagenet1000':224, 'CIFAR10':224, 'CIFAR100':224}[args.data_name]
    ...
    if args.data_name == 'imagenet1000' or args.data_name == 'imagenet100' :
        ...
        train_dataset = datasets.CIFAR10(
            args.dataset_dir,
            download=True,
            transform=sogclr.loader.TwoCropsTransform(transforms.Compose(augmentation1), transforms.Compose(augmentation1)),
        )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        save_path = os.path.join(save_root_path, logdir, 'checkpoint_%04d.pth.tar' % epoch)
